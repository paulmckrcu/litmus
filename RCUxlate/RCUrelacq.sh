...
# This program generates write-release and read-acquire instructions
# rather than memory barriers.
#
...
function emit_preamble(proc_num, line_in, line,  cs_num, g) {
	...
	for (g = 1; g <= ngp; ++g) {
		...
		if (rcugp_proc[g] == proc_num)
			continue;
		aux[proc_num ":" line++] = sprintf("r[acquire] r5%02d%02d gpstart%02d", g, cs_num, g);
	}
...
function emit_postamble(proc_num, line_in, line,  cs_num) {
	...
	aux[proc_num ":" line++] = sprintf("w[release] csend%02d 1", cs_num);
...
function emit_sync(proc_num, line_in, line,  gp_num, i) {
	...
	aux[proc_num ":" line++] = sprintf("w[release] gpstart%02d 1", gp_num);
	aux[proc_num ":" line++] = "f[mb]";
	for (i = 1; i <= ncs; ++i) {
		# Do not check critical sections in the same process
		if (rcucs_proc[i] == proc_num)
			continue;
		aux[proc_num ":" line++] = sprintf("r[acquire] r6%02d%02d csend%02d", i, gp_num, i);
	}
	aux[proc_num ":" line++] = "f[mb]";
